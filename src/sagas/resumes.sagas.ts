import { closeMonth, deleteResumes, getAll, post } from "../services/resumes.service";
...
function* onCallCloseResume(action: PayloadAction<{ id: string }>) {
    const { message } = yield call(closeMonth, action.payload.id);
    yield put(callGetAll())
}
...
export default [
    ...
    takeEvery(`${STATE_NAME}/callCloseResume`, onCallCloseResume),
];